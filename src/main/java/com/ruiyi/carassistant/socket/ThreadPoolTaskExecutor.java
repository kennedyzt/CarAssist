...
public class ThreadPoolTaskExecutor {
    ...
    public static int getPoolSize() {
        return executor != null ? ((ThreadPoolExecutor) executor).getPoolSize() : 0;
    }
    ...
    public static <T> T invoke(Runnable task, T result, TimeUnit unit, long timeout) throws TimeoutException, RuntimeException {
    ...
    public static <T> T invoke(Callable<T> task, TimeUnit unit, long timeout) throws TimeoutException, RuntimeException {
        ...
        try {
            /*
             * 这里的操作是确认任务是否已经完成，有了这个操作以后 1)对invoke()的调用线程变成了等待任务完成状态
             * 2)主线程可以接收子线程的处理结果
             */
            t = future.get(timeout, unit);
        } catch (TimeoutException e) {
    ...
    public static void invoke(ObdProtocol obdProtocol) {
        executor.submit(obdProtocol);
    }
